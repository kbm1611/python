...
print( sum(list( i for i in range(1, 101) if i % 5 == 0)) )
...
# (결과 예시: [1, 9, 25, 49, 81])
print(list( i*i for i in range( 1, 10, 2 ) ) )
...
for i, value in enumerate(sales):
    print("{}주차: {} {}만원".format(i+1, "★" * int(value / 100), value ) )
...
for i in range(len(car_numbers)):
    if(usage_minutes[i] <= 30):
        ...
        charge = 1000
        charge += int((usage_minutes[i] - 30) / 10) * 500
    
    if( charge > 20000 ): charge = 20000

    print("{}: {}분 주차, 최종 요금: {}".format(car_numbers[i], usage_minutes[i], charge))
...
while True:
    ...
    if( ch == 1 ):
        InputMoney = int(input("입금액>>"))
        balance += InputMoney
    elif( ch == 2 ):
        OutputMoney = int(input("출금액>>"))
